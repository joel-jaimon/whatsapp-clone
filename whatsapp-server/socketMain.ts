...
export const socketMain = async (io: any, socket: any) => {
  try {
    ...
    // Signin success state
    socket.on("callOtherUser", async (payload: any) => {
    ...
    socket.on("user-on-call", (roomId: string) => {
      socket.broadcast
        .to(roomId)
        .emit(
          "other-user-on-call",
    ...
    // Verification needed
    socket.on("updateUserProfile", async (payload: any) => {
    ...
    socket.on("updateOthersChats", async (payload: any) => {
      const {
